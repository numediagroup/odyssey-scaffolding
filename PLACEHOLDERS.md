...
| 3 | ~~Testimonials~~ | — | **Resolved by removal.** The three invented quotes and "Placeholder name" attributions have been deleted from `index.html`. Fake reviews on a live commercial site are unlawful under UK consumer law, so they were removed rather than shipped. The `.testi*` CSS and the null-guarded slider JS are still in place — to reinstate with **real** reviews, restore the section markup only. |
...
| 6 | ~~Contact form — submission method~~ | — | **Resolved.** All four forms POST to Formspree (`f/xljerlze`). `action`/`method` sit on the form so it still works with JavaScript disabled; `main.js` intercepts it to submit in the background and keep the visitor on the page. Honeypot field added for spam. |
...
| 24 | ~~Privacy policy — missing client facts~~ | — | **Resolved.** No placeholders remain on the page. Client confirmed there is no postal address, so the policy identifies the controller by trading name with phone/email/WhatsApp contact. Retention set to: kept while the enquiry is open, deleted once closed. |
| 26 | **ICO registration not stated** | `privacy-policy.html` | Deliberately not mentioned — the client has not confirmed whether Odyssey Scaffolding is registered with the ICO. Most UK businesses processing personal data must register and pay the annual fee (from £52). **Worth checking; if registered, add the number to "Who we are".** |
| 27 | **Retention vs. tax records** | `privacy-policy.html` · How long we keep information | The policy says information is deleted once the enquiry closes. If an enquiry becomes a paid job, HMRC requires business records to be kept for ~6 years, so that statement will not hold for customers who book work. **Recommend adding one line covering completed jobs.** |
| 25 | ~~Privacy policy must be rewritten if the form changes~~ | — | **Resolved.** Formspree is now live, and the policy has been updated: the "no server receives your data" claim is gone, Formspree is listed as a US-based processor, and retention now covers the copy held in the Formspree account. |
